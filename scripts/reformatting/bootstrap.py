...
cfg.MODEL.ROI_HEADS.SCORE_THRESH_TEST = 0 #0.05
cfg.MODEL.RETINANET.SCORE_THRESH_TEST = 0 #0.05
...
# Bootstrap dict with anti and non-anti curr dicts
bootstrap_dict = myEvaluator.EvaluateBootstrapper()

# Get out the two ordered dicts
curriculum_bootstrap_dict      = bootstrap_dict["curriculum"]
anti_curriculum_bootstrap_dict = bootstrap_dict["anti_curriculum"]

# Get the train dict
train_dicts = myDictGetters.getSharkTrainDicts()


### STANDARD CURRICULUM
# Sort by score from boostrap_dict
curriculum_train_dicts = []
# curriculum_bootstrap_dict is a *sorted* dictionary of filenames (in descending order of score))
# for each filename
for file_name,score in curriculum_bootstrap_dict.items():
  # find that filename in the train_dicts list
  for train_dict in train_dicts:
    if(train_dict["file_name"] == file_name):
      new_entry = train_dict
      break
  # append the new entry
  curriculum_train_dicts.append(new_entry)
...
# Sort by score from boostrap_dict
anti_curriculum_train_dicts = []
# anti_curriculum_bootstrap_dict is a *sorted* dictionary of filenames (in descending order of score))
# for each filename
for file_name,score in anti_curriculum_bootstrap_dict.items():
  # find that filename in the train_dicts list
  for train_dict in train_dicts:
    if(train_dict["file_name"] == file_name):
      new_entry = train_dict
      break
  # append the new entry
  anti_curriculum_train_dicts.append(new_entry)
